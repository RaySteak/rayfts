...
;(function($, doc, win){
    ...
    var Photobox, photobox, options, images=[], imageLinks, activeImage = -1, activeURL, lastActive, activeType, prevImage, nextImage, thumbsStripe, docElm, APControl, changeImage,
        ...
        overlay = $('<div id="pbOverlay">').append(
                        thumbsToggler = $('<input type="checkbox" id="pbThumbsToggler" unchecked hidden>'),
    ...
    Photobox.prototype = {
        ...
        imageLinksFilter : function(linksObj){
            ...
            function linksObjFilter(i){
                // search for the thumb inside the link, if not found then see if there's a 'that.settings.thumb' pointer to the thumbnail
                var link = $(this),
                    thumbImg,
                    thumbSrc = '';
                
                // Skip back button image
                var prevPath = normalizeDirUrl(window.location.href.split('/').slice(0, -2).join('/'));
                if(normalizeDirUrl(link[0].href) === prevPath)
                    return false;
                
                // Skip everything that is not an image
                if(!isImage(link[0].href))
                    return false;
            ...
            return [linksObj.filter(linksObjFilter), images];
        },
        ...
        events : {
            binding : function(){
                ...
                this.selector.on('click.photobox', this.target, function(e){
                    // !!!!!! This prevents clicking on the link
